...
import handleMemoClick2 from "../components/memoClick";

const animalEmojis = ["🐶", "🐱", "🐭", "🐰", "🦊", "🦁", "🐯", "🐻", "🐨", "🐼", "🦓", "🐴", "🦄", "🐮", "🐷", "🐸", "🐔", "🐦", "🐧", "🐢"];
const animalWords = ["dog", "cat", "mouse", "rabbit", "fox", "lion", "tiger", "bear", "koala", "panda", "zebra", "horse", "unicorn", "cow", "pig", "frog", "chicken", "bird", "penguin", "turtle"];
...
const Animals2 = () => {
  ...
  useEffect(() => {
    const shuffledArray = [...animalEmojis, ...animalWords];
    for (let i = shuffledArray.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [shuffledArray[i], shuffledArray[j]] = [shuffledArray[j], shuffledArray[i]];
    }
    const formattedArray = shuffledArray.map((emoji, i) => ({ index: i, emoji, word: animalWords[i], flipped: false }));
